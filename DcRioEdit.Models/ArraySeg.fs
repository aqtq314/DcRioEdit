...
module ArraySeg =
    ...
    let init length initiator =
        ofArray (Array.init length initiator)
    ...
    let ofIList (values : IList<'a>) =
        init values.Count (fun i -> values.[i])
